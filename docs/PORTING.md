...
- `SettingsStorageAdapter` は Chrome API の形ではなく、`SettingsStorageReader` / `SettingsStorageWriter` を合わせた `readAll` / `read` / `write` のキー値境界として移植先が実装する。
- `npm run build` は `tsconfig.core.json` で `src/core/` を Chrome 型なしでも検査し、純ロジックが `chrome.*` に依存しない状態を保つ。
...
1. `SettingsStorageAdapter` を実装し、既存キーを維持する。
   ...
   - `siteSpeeds`
   - `hasSeenOnboarding`
2. Chrome 版は `createChromeLocalStorageAdapter(chrome.storage.local)` で Chrome API を `SettingsStorageAdapter` に変換する。
3. iOS / Android 版は同じ `SettingsStorageAdapter` を満たすネイティブ保存アダプタを用意し、UI 層へ注入する。
4. UI 層からは保存先を直接触らず、アダプタの `readAll` / `read` / `write` を呼ぶ。
5. ページ本文の取得やバッジ表示など、プラットフォーム固有 API は UI / adapter 層に閉じ込める。
...
- 移植先アダプタは `SETTINGS_STORAGE_KEYS` 以外のキーを UI / core へ渡さない。既存データを読む場合は `pickSettingsStorageSnapshot()` と同等に既知キーだけを取り出す。
...
- `src/core/` を移植先で再利用するときは `src/core/**/*.ts` と必要な型だけを取り込み、Chrome 版の `popup.ts` / `content.ts` / `src/storage/chromeLocalStorage.ts` は含めない。
