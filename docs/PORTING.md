...
- `SettingsStorageAdapter` は Chrome API の形ではなく、移植先が実装する保存境界として扱う。
...
1. `SettingsStorageAdapter` を実装し、既存キーを維持する。
   ...
   - `siteSpeeds`
   - `hasSeenOnboarding`
2. UI 層からは保存先を直接触らず、アダプタの `getAll` / `get` / `set` を呼ぶ。
...
- ネイティブ保存先を使う場合も、`SettingsStorageAdapter` と同じ `getAll()` / `get(keys)` / `set(values)` の境界に合わせる。
- `getAll()` は `SETTINGS_STORAGE_KEYS` の全キーを返す。
- `get(keys)` は `readonly SettingsStorageKey[]` を受け取り、指定キーだけを返す。
- `set(values)` は `SettingsStoragePatch` を受け取り、渡されたキーだけを保存する。保存済みの他キーは消さない。
